...
export function BlueButton({ text, className }) {
    return (
        <motion.button whileTap={{ scale: 0.85 }} className={'btn btn-primary btn-xs max-w-xs border border-base-content/30 text-[14px] px-4 rounded-2xl ' + className}>{text.toUpperCase()}</motion.button>
    )
}
