...
export default function ResultsCard({ data }) {
    const { name, file, avatar, time } = data
    ...
    return (
        <div className='flex my-2 items-center justify-center gap-x-2 text-[14px] font-bold text-black border border-base-content/30 py-2 rounded-lg overflow-x-auto text-center'>
            ...
            <div className='w-1/4 text font-normal'>{file}</div>
            <div className='w-1/5 text-red-600'>{time}</div>
            <div className='w-1/5'><button className="btn btn-outline white-space w-auto btn-xs text-[12px] px-4 min-h-0 h-4 border border-base-content/30 rounded-lg text-black hover:bg-primary hover:border-none">VIEW RESULT</button> </div>
